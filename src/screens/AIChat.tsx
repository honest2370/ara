...
export default function AIChat() {
  ...
  // ——— start new chat + send first message ———
  const startChat = async (text: string) => {
    ...
    // Navigate first so loadMessages picks up the session
    replace({ name: "ai", params: { session: sid } });

    // Send the first message
    await processSend(text, sid);
  };
  ...
  return (
    <div className="relative flex h-dvh flex-col bg-white">
      ...
      {/* ——— Header ——— */}
      <header className="safe-top shrink-0 z-30 flex items-center gap-2 border-b border-slate-100 bg-white/90 backdrop-blur px-3 py-2.5">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar">
        {!sessionId ? (
          <div className="flex min-h-full flex-col items-center justify-center px-6 py-12 animate-fade">
            ...
            <div className="mt-8 grid w-full max-w-sm gap-2">
              {AI_SUGGESTIONS.map((s) => (
                <button key={s} onClick={() => startChat(s)}
                  className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left transition hover:border-accent-200 hover:bg-accent-50/50 active:scale-[.98]">
      ...
      {/* ——— Composer ——— */}
      <div className="safe-bottom shrink-0 border-t border-slate-100 bg-white px-3 pb-2.5 pt-2">
